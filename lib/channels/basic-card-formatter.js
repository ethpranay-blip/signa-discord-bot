...
// Action + direction label for the card header. Shows the actionable verb
// (BUY/SELL/HOLD/AVOID) with the directional bias in parens — "BUY (BULLISH)",
// "AVOID (BEARISH)" — so traders see the call, not just the bias. Falls back to
// the bare direction when action is unknown.
function actionLabel(call) {
  return call.action && call.action !== '—'
    ? `${call.action} (${call.direction})`
    : call.direction;
}

// Discord embed for live posting.
export function buildEmbed(call) {
  const header = `${call.ticker} · ${call.grade} · ${actionLabel(call)} · Conviction ${call.conviction ?? '—'}`;
