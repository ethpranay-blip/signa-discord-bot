...
export async function runChannelCycle(config, ctx) {
  ...
  const counts = { direction: 0, conviction: 0, levels: 0, triggers: 0, grade: 0, rr: 0, stage: 0 };
  let stageDegraded = 0;
  const passing = [];
  ...
  log(`   filter checks (passed/${enriched.length}): dir=${counts.direction} conv=${counts.conviction} levels=${counts.levels} trig=${counts.triggers} grade=${counts.grade} rr=${counts.rr} stage=${counts.stage}`);
