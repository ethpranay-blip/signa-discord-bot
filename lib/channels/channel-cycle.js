...
import { isNewSetup, remember } from './call-dedupe.js';
...
export async function runChannelCycle(config, ctx) {
  ...
  const calls = [];
  let suppressed = 0;   // repeats of a still-current setup (not logged as new CALLs)
  for (const e of top) {
    ...
    // Only log a CALL row when the setup is new or materially changed — a setup
    // that stays valid across cycles is the SAME trade, not a new one. Posting
    // to Discord is unaffected; this governs the paper-validation log only.
    const fresh = isNewSetup(call);
    if (!fresh) suppressed++;

    if (channelsEnabled) {
      await post(channel, buildEmbed(call), `${channel}-${call.ticker}`);
      if (fresh) remember(call);
      if (fresh) await track({
        type: 'call',
        channel,
  ...
  cycleRow.calls_logged = calls.length - suppressed;   // new/changed setups written to the log
  cycleRow.calls_repeat = suppressed;                  // same setup still valid from an earlier cycle
  if (channelsEnabled) await track(cycleRow);
